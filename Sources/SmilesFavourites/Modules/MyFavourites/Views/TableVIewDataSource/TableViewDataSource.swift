...
extension TableViewDataSource where Model == SwipeMessageViewModel {
    static func make(forSwipeMessage swipeMessage: SwipeMessageViewModel, reuseIdentifier: String = String(describing: SwipeMessageTableViewCell.self), data: String, isDummy: Bool = false) -> TableViewDataSource {
        return TableViewDataSource(
            models: [swipeMessage],
            reuseIdentifier: reuseIdentifier,
            data: data,
            isDummy: isDummy
        ) { (swipeMessage, cell, data, indexPath) in
            guard let cell = cell as? SwipeMessageTableViewCell else { return }
            cell.selectionStyle = .none
            cell.configureCell(with: swipeMessage)
        }
    }
}
...
extension TableViewDataSource where Model == FavouriteStackListResponse {
    static func make(forStackList stackList: FavouriteStackListResponse, viewModel: FavouritesStackListTableViewCell.ViewModel, reuseIdentifier: String = String(describing: FavouritesStackListTableViewCell.self), data: String, isDummy: Bool = false) -> TableViewDataSource {
        return TableViewDataSource(
            ...
            cell.selectionStyle = .none
            cell.configureCell(with: viewModel, shouldSkeleton: isDummy)
        }
    }
}
